...
use super::{
    AccountId, BalancesConfig, XORConfig, GenesisConfig, Signature, SudoConfig, SystemConfig,
    WASM_BINARY,
};
use sp_core::{ed25519, sr25519, Pair, crypto::AccountId32};
use sp_runtime::traits::{IdentifyAccount, Verify};

/// Helper function to generate a crypto pair from seed
fn get_from_seed<TPair: Pair>(seed: &str) -> TPair::Public {
    let pair = TPair::from_string(&format!("//{}", seed), None)
    ...
    GenesisConfig {
        ...
        pallet_balances_Instance1: Some(XORConfig {
            balances: endowed_accounts
                ...
                .cloned()
                .inspect(|x| { dbg!(x); })
                .filter(|x| x != &AccountId32::from([52u8, 45, 84, 67, 137, 84, 47, 252, 35, 59, 237, 44, 144, 70, 71, 206, 243, 67, 8, 115, 247, 189, 204, 26, 181, 226, 232, 81, 123, 12, 81, 120]))
                .inspect(|x| { dbg!(x); })
                .map(|k| (k, 1 << 60))
                .collect(),
        }),
