...
decl_event!(
    ...
    where
        ...
        BalanceOf = treasury::BalanceOf::<T>,
    {
        Ack(u8, AccId),
        IncomingTransfer(AccountId, AccId, BalanceOf),
        OutgoingTransfer(AccId, AccountId, BalanceOf),
    }
);
...
decl_module! {
    pub struct Module<T: Trait> for enum Call where origin: T::Origin {
        ...
        #[weight = 0]
        pub fn outgoing_transfer(origin, sender: T::AccountId, receiver: AccountId, amount: treasury::BalanceOf::<T>, nonce: u8) -> DispatchResult {
            debug::info!("called outgoing_transfer");
            let author = ensure_signed(origin)?;

            if Self::is_authority(&author) {
                <treasury::Module<T>>::burn(sender.clone(), amount);
                Self::deposit_event(RawEvent::OutgoingTransfer(sender, receiver, amount));
            }
        ...
        #[weight = 0]
        pub fn request_transfer(origin, receiver: AccountId, amount: treasury::BalanceOf::<T>, nonce: u8) -> DispatchResult {
            ...
            <treasury::Module<T>>::lock(from.clone(), amount);

            <Self as Store>::OcRequests::mutate(|v| v.push(OffchainRequest::OutgoingTransfer(amount, from, receiver, nonce)));
            Ok(())
        }

        #[weight = 0]
        pub fn incoming_transfer(origin, sender: AccountId, receiver: T::AccountId, amount: treasury::BalanceOf::<T>) -> DispatchResult {
            ...
            <treasury::Module<T>>::mint(receiver.clone(), amount);
            Self::deposit_event(RawEvent::IncomingTransfer(sender, receiver, amount));
            Ok(())
        }
        ...
        fn offchain_worker(block_number: T::BlockNumber) {
            for e in <Self as Store>::OcRequests::get() {
                match e {
                    OffchainRequest::OutgoingTransfer(amount, from, to, nonce) => {
                        let _ = Self::handle_outgoing_transfer(amount, from, to, nonce);
                    }
                }
            }
    ...
    */
                        ...
                        if to == bridge_account_id {
                            ...
                            if from == root_account_id {
                                ...
                                let result = signer.send_signed_transaction(|acc| {
                                    ...
                                    Call::incoming_transfer(from.clone(), recipient_account.clone(), amount)
                                });
                                ...
                                match result {
                                    Some((acc, Ok(_))) => {
                                        ...
                                        let external_asset_id = "XOR".into();
                                        let tx = ExternalTransaction {
                                            hash: "".into(),
                                            payload: vec![]
                                        };
                                        let instructions = vec![
                                            bridge::isi::handle_outgoing_transfer(&bridge_def_id, &external_asset_id, quantity, 0, &tx),
                                        ];
    ...
    fn handle_outgoing_transfer(
        ...
        let instructions = vec![
            ...
            //     asset,
            // )),
            bridge::isi::handle_incoming_transfer(&bridge_def_id, &external_asset_id, quantity, 0, to_account_id.clone(), &ExternalTransaction {
                hash: "".into(),
                payload: vec![]
            }),
        ];
        ...
        let results = signer.send_signed_transaction(|_acc| {
            Call::outgoing_transfer(from_account_id.clone(), to_account_id.clone(), amount, nonce)
        });
